...
for file in sorted(Path('squad2-en-translated-raw/html/').glob('*.html')):
    with open(file, 'r') as file:
        ...
        for elem in soup.body.children:
            ...
            # Get the answers
            if is_b(elem) and "number" in elem.get_text():
                para_id = int(
                    ''.join([i for i in elem.get_text().split() if i.isdigit()]))
            ...
            # Get questions
            if is_b(elem) and "Question" in elem.get_text():
...
with open("squad2_en_translated.json", "w") as json_file:
